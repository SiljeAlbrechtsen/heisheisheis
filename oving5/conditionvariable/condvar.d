...
class Resource(T) {
    ...
    T allocate(int id, int priority){
        mtx.lock();  // låser mutex

        queue.insert(id, priority);  // legger inn i proirity queue

        // så lenge vi ikke er først i køen
    ...
    void deallocate(T v){
        ...
        queue.popFront();  // Fjerner oss fra køen, fordi vi er ferdig
