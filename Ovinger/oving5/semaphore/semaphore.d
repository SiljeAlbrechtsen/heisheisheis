...
class Resource(T) {
    ...
    void deallocate(T v){
        ...
        value = v; //oppdaterer resursens verdi

        if(numWaiting[1]> 0){  //hvis noe med høy prioriet venter, så vekkes de først
            sems[1].notify(); 
        }

        else if(numWaiting[0] > 0){  //hvis noen med lav prioritet venter så vekkes dem
            sems[0].notify();
        }
        else{  // om ingen venter så er ressursen ledig
            busy = false;
        }

        mtx.notify();  //lås opp mutex
    }
}
